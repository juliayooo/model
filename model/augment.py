...
def augment(image, label):
    ...
    image = augmented["image"].astype(
        np.float32) / 255.0  # Normalize back
...
def tf_augment_fn(image, label):
    ...
    image.set_shape((224, 224, 3))  # Ensure TensorShape remains fixed
    return image, label
...
#     buffer_size=tf.data.AUTOTUNE)  # Optimize performance
...
for images, labels in train_dataset.take(1):
    print("THIS IS THe SHAPE", images.shape)  # Should print ("
